...
# from utils.training import evaluate_time as evaluate
from utils.training import evaluate
46/4: ex = sacred.Experiment('RRT Evaluation', ingredients=[data_ingredient, model_ingredient], interactive=True)
46/5:
# Filter backspaces and linefeeds
SETTINGS.CAPTURE_MODE = 'sys'
ex.captured_out_filter = apply_backspaces_and_linefeeds
...
resume = None
seed = 0
46/7: device = torch.device('cuda:0' if torch.cuda.is_available() and not cpu else 'cpu')
46/8: torch.manual_seed(seed)
46/9:
...
46/10: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), None'', 500)
46/11: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), None, 500)
46/12:
def read_file(filename):
    with open(filename) as f:
        lines = f.read().splitlines()
    return lines
46/13: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), None, 500)
...
def get_sets(desc_name, 
    ...
    return (train_set, query_train_set), (query_set, gallery_set)
46/16: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), None, 500)
46/17: from .dataset import FeatureDataset
46/18: from utilis.data.dataset import FeatureDataset
46/19: from utils.data.dataset import FeatureDataset
46/20: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), None, 500)
46/21: from torch.utils.data import DataLoader, RandomSampler, BatchSampler
46/22: random
46/23: sampler = 'random'
46/24:
if sampler == 'random':
...
recalls = [1, 5, 10]
46/28: num_candidates = 100
46/29: train_loader = DataLoader(train_set, batch_sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
46/30: query_train_loader = DataLoader(query_train_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/31: query_loader   = DataLoader(query_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/32: gallery_loader = DataLoader(gallery_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/33:
class MetricLoaders(NamedTuple):
    train: DataLoader
    num_classes: int
    ...
    query_train: DataLoader
    gallery: Optional[DataLoader] = None
46/34: from typing import NamedTuple, Optional
46/35:
class MetricLoaders(NamedTuple):
    train: DataLoader
    num_classes: int
    ...
    query_train: DataLoader
    gallery: Optional[DataLoader] = None
46/36: loaders, recall_ks = MetricLoaders(train=train_loader, query_train=query_train_loader, query=query_loader, gallery=gallery_loader, num_classes=len(train_set.categories)), recalls
46/37: torch.manual_seed(seed)
46/38:
from .matcher import MatchERT
from sacred import Ingredient
...
activation = "relu"  
normalize_before = False
46/43: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
46/44:
name = 'rrt'
seq_len = 1004
dim_K = 256
...
activation = "relu"
normalize_before = False
46/45: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
46/46:
if resume is not None:
   checkpoint = torch.load(resume, map_location=torch.device('cpu'))
   model.load_state_dict(checkpoint['state'], strict=True)
46/47: model.to(device)
46/48:  model.eval()
46/49: loaders.query.dataset.desc_name
46/50: loaders.query.dataset.data_dir
46/51: nn_inds_path = osp.join(loaders.query.dataset.data_dir, 'nn_inds_%s.pkl'%loaders.query.dataset.desc_name)
46/52: cache_nn_inds = torch.from_numpy(pickle_load(nn_inds_path)).long()
46/53:
eval_function = partial(evaluate, model=model, 
        cache_nn_inds=cache_nn_inds,
        recall=recall_ks, query_loader=loaders.query, gallery_loader=loaders.gallery)
46/54: metrics = eval_function()
46/55: oxford_gnd_file = "/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/oxford5k/gnd_roxford5k.pkl"
46/56: oxford_gnd = pickle_load(oxford_gnd_file)
46/57: oxford_gnd
46/58: oxford_gnd.keys()
46/59: oxford_gnd['gnd'][0]
46/60: metrics = eval_function()
46/61: gnd_file = "/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images/gnd_dev_viquae.pkl"
46/62: gnd = pickle_load(gnd_file)
46/63: gnd['gnd']
46/64: len(oxford_gnd['gnd'])
46/65: len(oxford_gnd['imlist'])
46/66: len(oxford_gnd['qimlist'])
46/67: len(gnd['qimlist'])
46/68: import numpy as np
46/69: index = np.random.choice(list(range(70)), size=len(gnd['qimlist']))
46/70: len(index)
46/71: index[:5]
46/72: gnd['gnd'] = oxford_gnd[index]
46/73: gnd['gnd'] = oxford_gnd[[index]]
46/74: gnd['gnd'] = oxford_gnd['gnd'][index]
46/75: gnd['gnd'] = oxford_gnd['gnd'][[index]]
46/76: gnd['gnd'] = oxford_gnd['gnd'][index]
46/77: index
46/78: gnd['gnd'] = [oxford_gnd['gnd'][i] for i in index]
46/79: gnd['gnd'][0]
46/80: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), 'gnd_dev_viquae', 500)
46/81: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), 'gnd_dev_viquae.pkl', 500)
46/82: from torch.utils.data import DataLoader, RandomSampler, BatchSampler
46/83:
if sampler == 'random':
   train_sampler = BatchSampler(RandomSampler(train_set), batch_size=batch_size, drop_last=False)
46/84: train_loader = DataLoader(train_set, batch_sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
46/85: query_train_loader = DataLoader(query_train_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/86: gallery_loader = DataLoader(gallery_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/87: loaders, recall_ks = MetricLoaders(train=train_loader, query_train=query_train_loader, query=query_loader, gallery=gallery_loader, num_classes=len(train_set.categories)), recalls
46/88: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
46/89: model.to(device)
46/90: model.eval()
46/91: cache_nn_inds = torch.from_numpy(pickle_load(nn_inds_path)).long()
46/92:
eval_function = partial(evaluate, model=model, 
        cache_nn_inds=cache_nn_inds,
        recall=recall_ks, query_loader=loaders.query, gallery_loader=loaders.gallery)
46/93: metrics = eval_function()
46/94: loaders
46/95: loaders.dataset.gnd
46/96: from utilis.data.utils import json_save, pickle_save
46/97: from utils.data.utils import json_save, pickle_save
46/98: pickle_save(gnd_file, gnd)
46/99: loaders.dataset.gnd
46/100: metrics = eval_function()
46/101: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images','test_query.txt', ('test_query.txt', 'test_gallery.txt'), 'gnd_dev_viquae.pkl', 500)
46/102:
if sampler == 'random':
   train_sampler = BatchSampler(RandomSampler(train_set), batch_size=batch_size, drop_last=False)
46/103: train_loader = DataLoader(train_set, batch_sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
46/104: query_train_loader = DataLoader(query_train_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/105: gallery_loader = DataLoader(gallery_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
46/106: loaders, recall_ks = MetricLoaders(train=train_loader, query_train=query_train_loader, query=query_loader, gallery=gallery_loader, num_classes=len(train_set.categories)), recalls
46/107: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
46/108: model.to(device)
46/109: model.eval()
46/110: cache_nn_inds = torch.from_numpy(pickle_load(nn_inds_path)).long()
46/111:
eval_function = partial(evaluate, model=model, 
        cache_nn_inds=cache_nn_inds,
        recall=recall_ks, query_loader=loaders.query, gallery_loader=loaders.gallery)
46/112: metrics = eval_function()
46/113: gnd['gnd'][0]
46/114: gnd_file
46/115: len(gnd['gnd'])
46/116: ? pickle_save
46/117: metrics = eval_function()
46/118: gnd['gnd'][0]['junk']
46/119: metrics = eval_function()
48/1: from utils import pickle_load, pickle_save, json_save, ReadSolution
46/120: %history -g -f /tmp/foo.py
46/121: pwd
46/122: %history -g -f foo.py
50/1:
from copy import deepcopy
from functools import partial
from pprint import pprint
...
# from utils.training import evaluate_time as evaluate
from utils.training import evaluate
50/4: ex = sacred.Experiment('RRT Evaluation', ingredients=[data_ingredient, model_ingredient], interactive=True)
50/5:
SETTINGS.CAPTURE_MODE = 'sys'
ex.captured_out_filter = apply_backspaces_and_linefeeds
50/6:
...
resume = None
seed = 0
50/7: device = torch.device('cuda:0' if torch.cuda.is_available() and not cpu else 'cpu')
50/8: torch.manual_seed(seed)
50/9:
def read_file(filename):
    with open(filename) as f:
        lines = f.read().splitlines()
...
def get_sets(desc_name, 
    ...
    return (train_set, query_train_set), (query_set, gallery_set)
50/13: from utils.data.dataset import FeatureDataset
50/14: from torch.utils.data import DataLoader, RandomSampler, BatchSampler
50/15: viquae_path = '/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images'
50/16: (train_set, query_train_set), (query_set, gallery_set) = get_sets('r50_gldv1', viquae_path, viquae_path,'test_query.txt', ('test_query.txt', 'test_gallery.txt'), None, 500)
50/17: sampler = 'random'
50/18:
if sampler == 'random':
   train_sampler = BatchSampler(RandomSampler(train_set), batch_size=batch_size, drop_last=False)
50/19:
...
if sampler == 'random':
   train_sampler = BatchSampler(RandomSampler(train_set), batch_size=batch_size, drop_last=False)
50/21: num_workers = 8  # number of workers used ot load the data \
50/22: pin_memory  = True  # use the pin_memory option of DataLoader
50/23: recalls = [1, 5, 10]
50/24: num_candidates = 100
50/25: train_loader = DataLoader(train_set, batch_sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
50/26: query_train_loader = DataLoader(query_train_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
50/27: query_loader   = DataLoader(query_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
50/28: gallery_loader = DataLoader(gallery_set, batch_size=test_batch_size, num_workers=num_workers, pin_memory=pin_memory)
50/29:
class MetricLoaders(NamedTuple):
    train: DataLoader
    num_classes: int
    ...
    query_train: DataLoader
    gallery: Optional[DataLoader] = None
50/30: from typing import NamedTuple, Optional
50/31:
class MetricLoaders(NamedTuple):
    train: DataLoader
    num_classes: int
    ...
    query_train: DataLoader
    gallery: Optional[DataLoader] = None
50/32: loaders, recall_ks = MetricLoaders(train=train_loader, query_train=query_train_loader, query=query_loader, gallery=gallery_loader, num_classes=len(train_set.categories)), recalls
50/33: torch.manual_seed(seed)
50/34:
from models.matcher import MatchERT
from sacred import Ingredient
model_ingredient = Ingredient('model', interactive=True)
...
activation = "relu"  
normalize_before = False
50/37: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
50/38:
name = 'rrt'
seq_len = 1004
dim_K = 256
...
activation = "relu"
normalize_before = False
50/39: model = get_model(num_global_features,num_local_features,seq_len,dim_K,dim_feedforward,nhead,num_encoder_layers,dropout,activation,normalize_before)
50/40:
if resume is not None:
   checkpoint = torch.load(resume, map_location=torch.device('cpu'))
   model.load_state_dict(checkpoint['state'], strict=True)
50/41: model.to(device)
50/42: model.eval()
50/43: loaders.query.dataset.desc_name
50/44: loaders.query.dataset.data_dir
50/45: nn_inds_path = osp.join(loaders.query.dataset.data_dir, 'nn_inds_%s.pkl'%loaders.query.dataset.desc_name)
50/46: cache_nn_inds = torch.from_numpy(pickle_load(nn_inds_path)).long()
50/47:
eval_function = partial(evaluate, model=model, 
        cache_nn_inds=cache_nn_inds,
        recall=recall_ks, query_loader=loaders.query, gallery_loader=loaders.gallery)
50/48: metrics = eval_function()
50/49: gnd_file = "/mnt/beegfs/home/smessoud/RerankingTransformer/models/research/delf/delf/python/delg/data/viquae_images/gnd_dev_viquae.pkl"
50/50: gnd = pickle_load(gnd_file)
50/51: gnd['gnd'][i]['junk']
50/52: gnd['gnd'][0]['junk']
50/53: num_samples = len(gnd)
50/54: num_samples
50/55: num_samples
50/56: cache_nn_inds
50/57: m_samples, top_k = cache_nn_inds.size()
50/58: num_samples, top_k = cache_nn_inds.size()
50/59:
for i in range(num_samples):
    junk_ids = gnd['gnd'][i]['junk']
50/60: top_k = min(100, top_k)
50/61: metrics = eval_function()
50/62: gallery
50/63: gallery_loader
50/64: gallery_loader[0]
50/65: gallery_global, gallery_local, gallery_mask, gallery_scales, gallery_positions, gallery_names = [], [], [], [], [], []
50/66:
...
top_k = min(100, top_k)
50/74: model.eval()
50/75: device = next(model.parameters()).device
50/76: to_device = lambda x: x.to(device, non_blocking=True)
50/77: query_global, query_local, query_mask, query_scales, query_positions, query_names = [], [], [], [], [], []
50/78: gallery_global, gallery_local, gallery_mask, gallery_scales, gallery_positions, gallery_names = [], [], [], [], [], []
50/79:
...
gallery_scales    = torch.cat(gallery_scales, 0)
gallery_positions = torch.cat(gallery_positions, 0)
50/83: torch.cuda.empty_cache()
50/84: device = next(model.parameters()).device
50/85:
query_global= query_global.to(device)
query_local = query_local.to(device)
query_mask  = query_mask.to(device)
query_scales= query_scales.to(device)
query_positions = query_positions.to(device)
50/86: num_samples, top_k = cache_nn_inds.size()
50/87: top_k = min(100, top_k)
50/88: medium_nn_inds = deepcopy(cache_nn_inds.cpu().data.numpy())
50/89:
for i in range(num_samples):
    junk_ids = gnd['gnd'][i]['junk']
...
for i in range(num_samples):
    ...
    new_ids = all_ids[new_ids]
    medium_nn_inds[i] = new_ids
50/92: medium_nn_inds = torch.from_numpy(medium_nn_inds)
50/93: scores = []
50/94:
